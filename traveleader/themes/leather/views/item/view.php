...
<!--  
    ...
    <div class="container_24">
                ...
                <ul id="idNum" class="hdnum">
                    <?php foreach ($item->itemImgs as $itemImg) {
                        if(!empty($itemImg->pic)){
                            $picUrl=$imageHelper->thumb('59','59',$itemImg->pic);
                        }else $picUrl=$item->getHolderJs('59','59');
                        echo '<li><img src="' .Yii::app()->baseUrl. $picUrl . '" width="59" height="59"></li>';
                    } ?>
                </ul>
            </div>
            <div style="width: 380px; height: 250px; overflow: hidden; position: relative;" id=idTransformView >
                <ul id=idSlider class=slider>
                    <?php foreach ($item->itemImgs as $itemImg) {
                        if($itemImg->pic){
                            $picUrl=$imageHelper->thumb('380','250',$itemImg->pic);
                        }else $picUrl=$item->getHolderJs('380','250');
                        //                        echo '<li><img src="' .$picUrl . '" width="450" height="450"></li>';
                        echo'<div><a href="javascript:void(0)" target="_blank" rel="nofollow"><img position=absolute   alt="' . $item->title . '" src="'  .Yii::app()->baseUrl.$picUrl . '" width="380" height="250"/></a></div>';
                    } ?>
                </ul>
            </div> 
             <div  class="deal_price_calendar">
                ...
                $this->widget('ext.EFullCalendar.EFullCalendar', array(
                		...
                		'options'=>array(
                				...
                				'lazyFetching'=>true,
                				//'events'=>Yii::app()->createUrl('item/CalendarEvents?id='.$item->item_id), // action URL for dynamic events, or
                				'events'=>$prices, // pass array of events directly
                				
                				// event handling
                				// mouseover for example
                				'eventMouseover'=>new CJavaScriptExpression('function(calEvent,jsEvent, view) {
										$(this).qtip({
           									overwrite: false,
        ...
        <script language=javascript>
            mytv("idNum", "idTransformView", "idSlider", 250, 4, true, 2000, 5, true, "onmouseover");
        ...
        <form action="<?php echo Yii::app()->createUrl('order/checkout'); ?>" method="post" class="deal_info" id="deal">     
            <div class="deal_price">
               <span class="font15"> 价格：</span> <span class="cor_red bold font30"><?php echo $item->currency . $item->price ?></span><span>起</span>
            </div>
            ...
            <div class="deal_destination">
            <span class="font15"> 出发地/目的地：</span> 
            ...
            <?php 
            	//$list = CHtml::listData($itemPrices, 'item_price_id', 'price_adult');
            	$priceArr = array();
            	foreach($itemPrices as $item_price){
            		$days = round((strtotime($item_price->date)-strtotime($today))/3600/24);
            		if( $days < 20){
						$priceArr[$item_price->item_price_id] =  
							$item_price->date. " " 
		                     .$item->currency . $item_price->price_adult . "/人 ; "
		                     .$item->currency . $item_price->price_child . "/儿童"; 
            		}
           		}
            	//echo CHtml::dropDownList('item_prices', $itemPrices,
            	//		$list,
            	//		array('empty' => '请选择出行日期'));
            ...
            <div class="deal_num">
                <span class="deal_num_c" content="no_cache">
                    ...
                    <a href="javascript:void(0)" class="add2"></a></span> <span>儿童</span>
